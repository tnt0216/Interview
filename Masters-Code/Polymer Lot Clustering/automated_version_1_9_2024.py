...
def load_embeddings():
    ...
    return sample_names, concatenated_data_df, frequency_data
...
def calculate_avg(data):
    """
    This function is taking the tests for the same material and averaging the results to provide one datapoint for
    each polymer in the clustering. It does this by defining ranges in the DataFrame to average for instance the first
    7 tests correspond to a polymer and need to be averaged and then the process is repeated for the other polymer
    tests. Once the ranges are defined the respective rows in the DataFrame are averaged and returned as a NumPy Array.
    The new averaged datapoint names are also returned from the function.
    """
    ...
    """Initializing an array to store the averaged column values"""
    averaged_dataset = []

    """Iterating through each specified range and calculating the averaged column values"""
    for start, end in ranges:
        """ Using .iloc for DataFrame slicing"""
        block = data.iloc[start:end + 1, :]
        averaged_dataset.append(block.mean(axis=0).to_frame().T)

    return pd.concat(averaged_dataset, ignore_index=True), avg_names
...
def run_clustering_analysis(data, num_components_range, sample_names, dataset_name):
    ...
    results. This basically just creates a constant to initialize the random number generator for k-means clustering. The
    value can be replaced by different integer values"""
    random_seed = 150
...
def calculate_distances(data, avg_names, datapoint, distance_metric='euclidean'):
    """
    ...
    This function returns a matrix which includes all the distances between the polymers and the names associated with
    the distances. The output of this function is placed in a table to determine what polymers are the closest to the
    specified datapoint.
    """
    ...
    for i, (index, row) in enumerate(data.iterrows()):
        ...
        distances.append((avg_names[i], distance))
...
def main():
    ...
    sample_names, concatenated_data, frequency_data = load_embeddings()
    ...
    """Averaging all of the same tests to have single data points for each polymer"""
    averaged_dataset, avg_names = calculate_avg(concatenated_data)
    print("Here are the averaged data points:")
    print(averaged_dataset)

    max_components_average = min(averaged_dataset.shape[0], averaged_dataset.shape[1])
    num_components_range_average = range(1, max_components_average + 1)

    print("Running Clustering Analysis on Averaged Dataset:")
    run_clustering_analysis(averaged_dataset, num_components_range_average, avg_names, data2)

    """Getting the lot names associated with the averaged values"""
    averaged_excel = pd.concat([pd.DataFrame(avg_names), averaged_dataset], axis=1)
    print(averaged_excel)

    """Saving the Averaged Dataset to an Excel file"""
    averaged_excel.to_excel(r'C:\Users\tnt02\OneDrive\Desktop\Masters Research\Running Data\Last Run\Excel '
                            r'Files\Averaged_Dataset.xlsx', sheet_name='sheet1', index=False)

    """Making distance table take measure how far away each averaged point is to lot 30035"""
    datapoint = averaged_dataset.iloc[6]
    distances = calculate_distances(averaged_dataset, avg_names, datapoint, distance_metric='euclidean')

    """Creating a DataFrame with distances"""
    print("\nDistance from lot 30035:")
    distance_table = pd.DataFrame(distances, columns=['Lot', 'Distance to 30035'])
    distance_table = distance_table.sort_values(by='Distance to 30035')
    print(distance_table)

    """Saving the DataFrame to an Excel file"""
    distance_table.to_excel(
        r'C:\Users\tnt02\OneDrive\Desktop\Masters Research\Running Data\Last Run\Excel '
        r'Files\Average_Euclidean_Distance_Table.xlsx', sheet_name='sheet1', index=False)
    ...
    run_clustering_analysis(concatenated_data, num_components_range_original, sample_names, data1)
